...
void CoreTP1::Render(double dt)
{
    ...
    //ENEMYSHIP
    //Check collisions avec vaisseau ennemi
    for (std::list<std::unique_ptr<EnemyShip>>::iterator it = enemies.begin() ; it != enemies.end(); ++it)
        if((*it)->checkCollisionWith(spaceship)) (*it)->setNotAlive();
    //TODO : Check collisions missiles avec vaisseaux ennemis
    //Enlever les morts
        for (std::list<std::unique_ptr<EnemyShip>>::iterator it = enemies.begin() ; it != enemies.end(); ++it)
          if(!(*it)->isAlive()) {
           // it = enemies.erase(it);
    ...
    //Afficher les ennemis
        ...
        for (std::list<std::unique_ptr<EnemyShip>>::iterator it = enemies.begin() ; it != enemies.end(); ++it)
            if((*it)->canShoot()) {
                std::vector<glm::vec3> listTmp = (*it)->getPosition();
                for (std::vector<glm::vec3>::iterator itbis = listTmp.begin() ; itbis != listTmp.end(); ++itbis)
                    listMissiles.push_back(std::unique_ptr<Projectile>(new Projectile((*itbis).x, (*itbis).y, (*itbis).z, 0,0,-12)));
            }


        //ENEMYSHIPBIS
        //Check les collisions avec vaisseau ennemi
        for (std::list<std::unique_ptr<EnemyShipBis>>::iterator it = enemiesBis.begin() ; it != enemiesBis.end(); ++it)
            if((*it)->checkCollisionWith(spaceship)) (*it)->setNotAlive();
        //TODO : Check collisions missiles avec vaisseaux ennemis
        //Enlever les morts
            for (std::list<std::unique_ptr<EnemyShipBis>>::iterator it = enemiesBis.begin() ; it != enemiesBis.end(); ++it)
              if(!(*it)->isAlive()) {
              //  it = enemiesBis.erase(it);
              //  it--; 
			  }
        //Popper éventuellement un nouvel ennemi
        ...
        //Afficher les ennemis
            ...
            for (std::list<std::unique_ptr<EnemyShipBis>>::iterator it = enemiesBis.begin() ; it != enemiesBis.end(); ++it)
                if((*it)->canShoot()) {
                    std::vector<glm::vec3> listTmp = (*it)->getPosition();
                    for (std::vector<glm::vec3>::iterator itbis = listTmp.begin() ; itbis != listTmp.end(); ++itbis)
                        listMissiles.push_back(std::unique_ptr<Projectile>(new Projectile((*itbis).x, (*itbis).y, (*itbis).z, 0,0,-16)));
                }
            //TODO : Check collision vaisseau et missiles ennemis
            //Lancer les missiles !!!
            for (std::list<std::unique_ptr<Projectile>>::iterator it = listMissiles.begin() ; it != listMissiles.end(); ++it)
              (*it)->render(dt);
}
