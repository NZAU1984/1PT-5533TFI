...
EnemyShip::EnemyShip() :
    ...
    _angle(0),
    _zPosition(( rand()/(double)RAND_MAX ) * (120-100) + 100),
    _xPosition(( rand()/(double)RAND_MAX ) * (40-0) + 0 - 20)
{
...
_addShapeToList(&mainBackConnector);
_addShapeToList(&mainBackMotorsConnector);
_addShapeToList(&firstBackConnector);
_addShapeToList(&secondBackConnector);
_addShapeToList(&firstMotor);
_addShapeToList(&secondMotor);
_addShapeToList(&mainCanon);
_addShapeToList(&leftFrontConnector);
_addShapeToList(&rightFrontConnector);
_addShapeToList(&supportLeft);
_addShapeToList(&supportRight);
_addShapeToList(&leftCanon);
_addShapeToList(&rightCanon);
_addShapeToList(&rightRightCanon);
_addShapeToList(&leftLeftCanon);
...
void EnemyShip::goForward(float zMove){
    if(_zPosition < -20) { alive=false; }
    _zPosition += zMove;
    mainBody.SetTransform(glm::translate(glm::mat4(), vec3(_xPosition,0,_zPosition)));
//float posx = _xPosition - (frontConnectorHeight/2 + mainBodyEnemyRadius) * cos(-(glm::pi<float>()/3 -alpha));
//float posz = _zPosition-frontConnectorHeight/2;
//    _LOG_INFO() << "middle left" << posx << ", " << posz;
//posx = _xPosition + (frontConnectorHeight/2 + mainBodyEnemyRadius) * cos(glm::pi<float>()/3 -alpha);
//posz = _zPosition-frontConnectorHeight/2;
//    _LOG_INFO() << "middle right" << posx << ", " << posz;
//posx = _xPosition - (mainBodyEnemyRadius+frontConnectorHeight) * cos(-glm::pi<float>()/6);
//posz = _zPosition-frontConnectorHeight/2;
//    _LOG_INFO() << "extrem left" << posx << ", " << posz;
//posx = _xPosition + (mainBodyEnemyRadius+frontConnectorHeight) * cos(glm::pi<float>()/6);
//posz = _zPosition-frontConnectorHeight/2;
//    _LOG_INFO() << "extrem right" << posx << ", " << posz;
}
...
std::vector<glm::vec3> EnemyShip::getPosition()
{
    ...
    theVector.push_back(vec3( _xPosition - (frontConnectorHeight/2 + mainBodyEnemyRadius) * cos(-(glm::pi<float>()/3 -alpha)), 0, _zPosition-frontConnectorHeight/2)); //middle left
    theVector.push_back(vec3(_xPosition + (frontConnectorHeight/2 + mainBodyEnemyRadius) * cos(glm::pi<float>()/3 -alpha), 0, _zPosition-frontConnectorHeight/2));  //middle right
    theVector.push_back(vec3(_xPosition - (mainBodyEnemyRadius+frontConnectorHeight) * cos(-glm::pi<float>()/6), 0, _zPosition-frontConnectorHeight/2)); //extrem left
    theVector.push_back(vec3(_xPosition + (mainBodyEnemyRadius+frontConnectorHeight) * cos(glm::pi<float>()/6), 0, _zPosition-frontConnectorHeight/2));
    theVector.push_back(vec3(_xPosition,0,_zPosition));
...
{
    if((_lastShot + _delay) < glfwGetTime()) {
        updateLastShot();
        return true;
    }
    return false;
...
void EnemyShip::updateLastShot()
{
    _lastShot = glfwGetTime();
}
