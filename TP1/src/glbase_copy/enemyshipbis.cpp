...
EnemyShipBis::EnemyShipBis() :
    ...
    _angle(0),
    _zPosition(( rand()/(double)RAND_MAX ) * (150-120) + 120),
    _xPosition(( rand()/(double)RAND_MAX ) * (30-0) + 0 - 15)
{
...
std::vector<glm::vec3> EnemyShipBis::getPosition()
{
    std::vector<glm::vec3> theVector;

    theVector.push_back(vec3( _xPosition-5, 0, _zPosition+0.40f-canonLength)); //middle left
    theVector.push_back(vec3(_xPosition-7, 0, _zPosition+0.40f-canonLength));  //extrem right
    theVector.push_back(vec3(_xPosition+5, 0, _zPosition+0.40f-canonLength)); //middle left
    theVector.push_back(vec3(_xPosition+7, 0, _zPosition+0.40f-canonLength));
...
void EnemyShipBis::updateLastShot()
{
